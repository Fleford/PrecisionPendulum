...
# log = np.loadtxt("7_05_pm_1_15_2020.tsv")
# x = log[:, 1]
...
#
# peak_time_list = peak_time_list[3000:]
peak_time_list = np.loadtxt("virtual_peak_times.txt")
# print(len(peak_time_list))

# Reports the difference between adjacent elements
for offset in range(1, len(peak_time_list) - 100):
    difference_list = []
    # offset = 60
    for i in range(offset, len(peak_time_list)):
        # if peak_time_list[i] - peak_time_list[i-offset] > 5000:
    ...
    print(offset, np.std(difference_list)/np.mean(difference_list))
